...
// The original file was double-encoded and then powershell Get-Content > temp.txt
// caused specific emojis to be transliterated lossily. We map them back perfectly.
let txt = fs.readFileSync('temp.txt', 'utf16le');

// Pre-recovery for completely mangled emojis:
txt = txt.replace(/ðY\'o/g, '💕');
txt = txt.replace(/ðYZµ/g, '🎵');
txt = txt.replace(/ðYO\./g, '🌅');
txt = txt.replace(/ðYZ┼/g, '🎆');
txt = txt.replace(/â┼\'/g, '→');
txt = txt.replace(/â┼\?/g, '←');
txt = txt.replace(/âœ¨/g, '✨');
txt = txt.replace(/ðY"\?/g, '🔀'); 
txt = txt.replace(/â\?®/g, '⏭');
txt = txt.replace(/â\?­/g, '⏮');
txt = txt.replace(/ðYO´/g, '🌴');
// Also, the heart emoji in "Mensagem especial"
txt = txt.replace(/\?ϋ\?/g, '❤️'); 
...
let buf = Buffer.alloc(txt.length * 4);
let offset = 0;
for (let i = 0; i < txt.length; i++) {
  let char = txt[i];
  // If we already replaced it with an emoji, don't run it through latin1 decoding
  // Emojis are > 0xFF, but latin1 decode is ONLY for the double-encoded bytes
  let code = char.charCodeAt(0);
  if (code > 0xFF && win1252ToByte[char] === undefined) {
    // This is a native unicode character (e.g. an emoji we just restored)
    // Write it as UTF-8 bytes to the buffer
    let tempBuf = Buffer.from(char, 'utf8');
    tempBuf.copy(buf, offset);
    offset += tempBuf.length;
  } else {
    // It's a double-encoded byte
...
// Fix the goTo for Abra seu presente
html = html.replace(
...
console.log("Restoration 2 Complete!");
